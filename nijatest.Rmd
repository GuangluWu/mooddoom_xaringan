...
output:
  xaringan::moon_reader:
    ...
    css: [default, "mooddoom.css", default-fonts]
...
background-image: url(https://upload.wikimedia.org/wikipedia/commons/2/23/Caltech_chemical_laboratory_1923.png)
...
Image credit: [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Caltech_chemical_laboratory_1923.png)
...
> This is a comment in this slide which you can't see.

<ru-blockquote> `[//]: # (There is a comment below)` </ru-blockquote> 

[//]: # (I'm a comment)

<ru-blockquote> `[//]: # (There is a comment above)` </ru-blockquote> 
---
class: center, middle
name: dreamland

# Slide label to be cited in next slide
...
And this slide will not be counted (the bottomright is the same as the last slide)
---
template: dreamland

You can also template the [**labelled slide**](#dreamland)
---
layout: true
class: inverse, center, middle
...
### To stop layout: add .red[layout: false] in next slide
...
.pull-left[<iframe width="560" height="315" src="https://www.youtube.com/embed/8fKKiaSLLEY" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>]

.pull-right[
Video code obtained from Youtube share menu
]
...
Video embed code from YouTube share menu
...
---
background-image: url(image/karl-moustache.jpg)
...
# slight diffent slides to test pull left

.pull-left[
.pull-left[Provides an R Markdown output format ]

.pull-right[as a wrapper for remark.js, and you can use it in the YAML metadata, e.g.]
]
...
See the help page `?xaringan::moon_reader` for all possible options that you can use.<sup>up</sup><sub>down</sub>
]
...
Notice! the next increments take place within a sentence.
...
.pull-left[
![THis is a image](image/karl-moustache.jpg)

.journal[Karl et al.  
[*Journal of No Peer Review*, **2020**, *24(4)*, 9-18](https://www.google.com)]
]
...
# Another example
This example use `backgroup-position` to set image to right
...
- This .red[can] .blue[replicate] my 'float .grey[right]' hack
- We can control the 
- size of the image
- by using the 
...
We can do this with white, .grey[grey], .black[black], .red[red] and .blue[blue]. 

--

To really make text stand out we can just use \*\*text\*\* like **this**.

this example use `class: middle, center` to realise **"starwar effect"**
...
- When you use the "Infinite Moon Reader" addin in RStudio, your R session will be blocked by default. You can click the red button on the right of the console to stop serving the slides, or use the _daemonized_ mode so that it does not block your R session. To do the latter, you can set the option
   ...
   <ru-blockquote> in your current R session, or in `~/.Rprofile` so that it is applied to all future R sessions. I do the latter by myself.</ru-blockquote>
...
```{r, highlight.output=c(1, 3), echo=FALSE, comment='#'}
head(iris)
```
...
If you have installed Google Chrome or Chromium, you can use the function   .`agedown::chrome_print()]`to print an HTML page to PDF:
...
.pull-left[hook cause some problem, use a method from [here](https://stackoverflow.com/questions/31324277/you-must-enable-javascript-to-view-this-page-properly-error-knitr-rgl)]
